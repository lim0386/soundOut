...
int scale_select = 3; //<-----pick the tuning you want here
...
//  fill(10,10,10, 40);  rect(0,0,width,height);  //draws a rectangle with alpha-40 so you get a fade effect
  ...
  int interval= 1; 
  float viewscale; 
  float wide=2*width/3;
  float point1; 
  float point2; 
  float point3; 
  float point4;
  float wavesize=height/18;  
  float smallwavesize=wavesize/2; 
  float num_strings = Scales[scale_select][0];
